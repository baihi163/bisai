...
def plot_previews(ts: pd.DataFrame, ev: pd.DataFrame) -> None:
    ...
    ax1.set_xlabel("时间")
    ax1.set_title("基准策略预览：负荷 / 电动汽车充电 / 光伏 / 购电")
    ...
    ax1.set_xlabel("时间")
    ax1.set_title("基准策略预览：储能状态与充放电功率")
    ...
    ax1.set_xlabel("时间")
    ax1.set_title("基准策略预览：购电功率与购电电价")
    ax1.legend(loc="upper left", fontsize=8)
    ax2.legend(loc="upper right", fontsize=8)
    ax1.grid(True, alpha=0.3)
    fig.autofmt_xdate()
    ...
    axes[1].set_xlabel("能量缺口（kWh）")
    axes[1].set_ylabel("车辆数")
    fig.suptitle("基准策略预览：电动汽车会话汇总", fontsize=12)
    fig.tight_layout()
