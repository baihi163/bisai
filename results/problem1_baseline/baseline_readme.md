...
2. **电动汽车**：到站即充——在连接时段内若电量未达离站目标，则按 `min(最大充电功率, 目标与容量约束下的功率上界)` 充电；**不向园区放电**；不考虑电价与光伏余量；未达标会话仅标记，不中断仿真。
3. **固定储能**：
   - 仅使用**剩余光伏**（满足负荷与 EV 后的交流剩余）以效率 `charge_efficiency` 充电；
   - 当购电价处于**全周离散电价中的最高一档**（若全周仅单一电价则**不触发**高价放电）且存在功率缺口时，按效率 `discharge_efficiency` 放电以降低购电；
   - 其余时段静置（不从电网充电）。
...
对每个 15 min：`EV 充电功率` → `总需求 = 原生负荷 + EV 充电` → `光伏供本地` → `高价则储能放电补缺口` → `购电` → `剩余光伏充储能` → `上网与弃光`。
...
| ev_total_charge_kw | EV 总充电功率（交流侧需求） |
...
- EV 能量：`ΔE = P_charge × Δt`（kW×h），**未使用**车载充电效率（输入数据未提供）。
- 高价档判定依赖购电价的**数值分档**；若全天单一电价，储能**不会**按「高价放电」规则动作。
- 未建立交流潮流与损耗模型；功率瞬时平衡。
